...
export function BottomNav() {
  ...
  return (
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-[#07070a]/95 backdrop-blur-xl border-t border-white/10 pb-safe pt-2 px-4 select-none shadow-[0_-8px_24px_rgba(0,0,0,0.7)]">
          ...
          if (item.isPrimary) {
            return (
              ...
              >
                <div
                  className={cn(
                    "w-13 h-13 rounded-2xl flex items-center justify-center shadow-xl transition-all duration-300",
                    isActive
                      ? "bg-gradient-to-tr from-amber-400 to-yellow-500 text-zinc-950 shadow-amber-500/40 ring-4 ring-[#07070a] scale-110"
                      : "bg-amber-500 text-zinc-950 shadow-amber-500/25 ring-4 ring-[#07070a] group-hover:scale-105 active:scale-95"
                  )}
                >
                  <Icon className="w-6 h-6 fill-zinc-950" />
                </div>
                <span
                  className={cn(
                    "text-[10px] font-black mt-1 transition-colors tracking-tight",
